...
part_verts, _ = read_obj("voca_lower_face.obj")
...
print(len(indices))
with open("voca_lower_face.txt", "w") as fp:
    fp.write(" ".join(str(x) for x in indices))
print(len(non_indices))
with open("voca_non_lower_face.txt", "w") as fp:
    fp.write(", ".join(str(x) for x in non_indices))
print(len(non_faces_idx))
with open("voca_non_lower_face_faces.txt", "w") as fp:
